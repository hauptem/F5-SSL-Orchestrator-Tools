...
- Search drift detection now compares datagroup values, not just keys. Entries present on every host but holding different values are flagged as a value mismatch, with per-host values shown in both Diff and Search results; previously the same key with different data on two hosts reported as consistent. Each entry is classified once after the fleet pull, so Diff and Search always agree. Search patterns still match keys only. URL categories are unaffected - they have no values. Applies to both versions (bash receives this as a targeted bugfix ... its pull loop already parsed values and discarded them)
...
- 5.3 is the final bash release; the PowerShell version will be the only one maintained going forward - however at 5.3 all core features have been implemented. Any further updates to the bash version will be targeted bugfixes
