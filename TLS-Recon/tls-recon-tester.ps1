...
function Read-Ports {
    ...
    while ($true) {
        ...
        switch ($choice) {
            ...
            "3" {
                if ($Script:LastPorts.Count -gt 0) {
                    Write-Success "Using previous port list"
                    return $Script:LastPorts
                } else {
                    Write-Error-Msg "Invalid selection. Enter 1 or 2."
                }
            }
            default {
                if ($Script:LastPorts.Count -gt 0) {
                    Write-Error-Msg "Invalid selection. Enter 1, 2, or 3."
                } else {
                    Write-Error-Msg "Invalid selection. Enter 1 or 2."
                }
            }
...
function Read-PortsInteractive {
    Write-Host ""
    Write-Info "Enter ports one at a time. Type 'done' when finished."
    Write-Host ""
    ...
    while ($true) {
        if ($count -eq 0) {
            ...
            $port = Read-Host -Prompt "Enter port (or 'done' to finish)"
        }
        
        if ($port -match '^[Dd](one)?$') {
            if ($count -eq 0) {
                Write-Error-Msg "You must enter at least one port"
                continue
            }
            break
        }
        ...
        if ([string]::IsNullOrWhiteSpace($port)) {
            if ($count -eq 0) {
                ...
                Write-Warning-Msg "Empty input ignored. Type 'done' to finish."
            }
            continue
        }
        
        if (Test-ValidPort $port) {
            if ($ports -contains $port) {
                Write-Warning-Msg "Port $port already added, skipping duplicate"
            } else {
                $ports += $port
                $count++
                Write-Success "Added port $port ($count ports total)"
            }
        }
    }
...
function Test-TCPPorts {
    ...
    foreach ($port in $Ports) {
        ...
        } -ArgumentList $Target, $port, $Script:Timeout
    }
