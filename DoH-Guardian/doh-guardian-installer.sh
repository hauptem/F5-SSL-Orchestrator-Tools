...
# Version 1.0
#
# Based entirely on Kevin Stewart's SSLO Service Extensions "DoH Guardian"
...
# This method is not acceptable for closed network customers so the curl dependancy
# was removed and the individual artifact files were rolled into a single installer
# script for ease of use and extension portability. The optional sinkhole
# configuration is also folded into this single script and offered as an optional
# step at the end of the core install.
#
# This script also offers a full uninstaller for all created components,
# including the optional sinkhole objects if they are present.
...
render_banner() {
    ...
    echo -e "${C_BOLD}${C_BLUE}║${C_RESET}  ${C_BOLD}${C_WHITE}DoH Guardian v1.0${C_RESET}                                           ${C_BOLD}${C_BLUE}║${C_RESET}"
    echo -e "${C_BOLD}${C_BLUE}╚══════════════════════════════════════════════════════════════╝${C_RESET}"
    echo
}
...
## step_done - advance the progress bar 
step_done() {
...
## result. Matches the REST helper output style so the log stream looks
## uniform regardless of which channel was used.
## ===========================================================================
tmsh_run() {
    local description="$1"
    shift
...
## the extracted temp files. If COMPLETED never flipped to 1 and mutations
## had started, the script was interrupted mid-run and we warn loudly so
## the operator knows the BIG-IP may be in a partial state.
## ===========================================================================
COMPLETED=0
MUTATIONS_STARTED=0
cleanup_on_exit() {
    local rc=$?
    rm -f rule-converter.py doh-guardian-rule.out
    rm -f doh-guardian-rule doh-guardian-service
...
if [[ "${MODE}" == "1" ]]; then
    ...
    INSTALL_TOTAL=9
    echo -e "${C_WHITE}Objects to be removed (anything not present will be skipped):${C_RESET}"
    echo -e "${C_WHITE}  - iAppsLX block:         ssloS_DoH_Guard${C_RESET}"
    echo -e "${C_WHITE}  - LTM iRule:             doh-guardian-rule${C_RESET}"
...
if [[ "${MODE}" == "2" ]]; then
    ...
    ## Phase 1: Discovery
    ## -----------------------------------------------------------------------
    step_screen "Discover existing DoH Guardian and sinkhole components"
    info "Scanning the BIG-IP for DoH Guardian and sinkhole objects"
    ...
    if (( HAS_SINK_VIRTUAL )); then
        echo -e "  ${C_WHITE}* sinkhole-internal-vip:        found${C_RESET}"
    else
        echo -e "  ${C_WHITE}* sinkhole-internal-vip:        not present${C_RESET}"
    fi
    ...
    ## -----------------------------------------------------------------------
    ## Phase 5: Delete sinkhole-target-rule iRule (REST, matching Kevin's
    ...
    ## -----------------------------------------------------------------------
    ## Phase 6: Delete sinkhole-internal-vip virtual (tmsh, matching Kevin's
    ## create path for this object)
    ## -----------------------------------------------------------------------
...
clear
render_banner
info "Scanning the BIG-IP for existing DoH Guardian or sinkhole components"
info "This may take a moment"
echo

EXISTING=()
if rest_get "/mgmt/tm/ltm/rule/doh-guardian-rule"; then
    EXISTING+=("LTM iRule: doh-guardian-rule")
fi
if rest_get "/mgmt/tm/ltm/rule/sinkhole-target-rule"; then
    EXISTING+=("LTM iRule: sinkhole-target-rule")
fi
if rest_get "/mgmt/tm/ltm/virtual/sinkhole-internal-vip"; then
    EXISTING+=("LTM virtual: sinkhole-internal-vip")
fi
if rest_get "/mgmt/tm/ltm/profile/client-ssl/sinkhole-clientssl"; then
    EXISTING+=("LTM client-ssl: sinkhole-clientssl")
fi
if rest_get "/mgmt/tm/sys/file/ssl-cert/sinkhole-cert"; then
    EXISTING+=("SSL cert: sinkhole-cert")
fi
if rest_get "/mgmt/tm/sys/file/ssl-key/sinkhole-cert"; then
    EXISTING+=("SSL key: sinkhole-cert")
fi
existing_block=$(curl -sk -u "${BIGUSER}" \
    "https://localhost/mgmt/shared/iapp/blocks" \
    | jq -r '.items[] | select(.name=="ssloS_DoH_Guard") | .id' 2>/dev/null)
if [[ -n "${existing_block}" && "${existing_block}" != "null" ]]; then
    EXISTING+=("iAppsLX block instance: ssloS_DoH_Guard")
fi

if (( ${#EXISTING[@]} > 0 )); then
    fail "Existing DoH Guardian or sinkhole components detected:"
    echo
...
## ===========================================================================
## Optional sinkhole install - prompt the user
## ===========================================================================
clear
render_banner
render_progress
...
echo -e "${C_WHITE}  - LTM iRule:        sinkhole-target-rule${C_RESET}"
echo
read -p "$(echo -e "${C_WHITE}Install sinkhole components? (y/n): ${C_RESET}")" sinkhole_choice
echo

SINKHOLE_INSTALLED=0
if [[ "${sinkhole_choice}" =~ ^[Yy]$ ]]; then
    SINKHOLE_INSTALLED=1
    INSTALL_TOTAL=12

    ## -----------------------------------------------------------------------
    ## Sinkhole Step 1: Generate cert and key with openssl (Kevin's exact
    ## invocation - empty Subject, SAN-only via SSL Orchestrator forging)
    ## -----------------------------------------------------------------------
...
extendedKeyUsage=serverAuth,clientAuth") > /dev/null 2>&1
    ...
    ## transaction (Kevin's exact sequence)
    ## -----------------------------------------------------------------------
    ...
    ## -----------------------------------------------------------------------
    ## Sinkhole Step 3: Create client-ssl profile (tmsh, Kevin's exact command)
    ## -----------------------------------------------------------------------
    ...
    ## -----------------------------------------------------------------------
    ## Sinkhole Step 4: Create internal virtual server (tmsh, Kevin's exact
    ## command)
    ## -----------------------------------------------------------------------
    ...
    ## -----------------------------------------------------------------------
    ## Sinkhole Step 5: Create sinkhole-target-rule iRule (REST, Kevin's
    ## exact rule content)
    ## -----------------------------------------------------------------------
