...
function Get-BlockDependencyRefs {
    ...
    switch ($DeploymentType) {
        ...
        "TOPOLOGY" {
            ...
            # Gateway pool: egressNetwork.gatewayPool
            if ($ConfigData.egressNetwork -and $ConfigData.egressNetwork.gatewayPool) {
                $gwPool = $ConfigData.egressNetwork.gatewayPool
...
function Invoke-BlockPost {
    ...
    Write-LogWarn "$Label - no block ID returned"
    return $true
}
...
function Invoke-SsloRestore {
    ...
    if ($targetResult.Success -and $targetResult.Response.items) {
    ...
    # -------------------------------------------------------------------------
    Write-Host ""
    $confirm = Read-Host "  Type CONFIRM to proceed with replay (anything else cancels)"
    ...
    foreach ($item in $restorePlan) {
        ...
        if ($blockId) {
            ...
            Write-LogWarn "$($item.DeploymentName) - no block ID returned"
            $restored += $item.DeploymentName
        }
    ...
    Write-Host ""
    if ($failed.Count -eq 0 -and $restored.Count -gt 0) {
